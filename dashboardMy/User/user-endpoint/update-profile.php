...
if (isset($_POST['customer_ID'])) {
    ...
    try {
        // 检查是否有重复的 customer_Name
        $stmt = $conn->prepare("SELECT * FROM `customer` WHERE `customer_Name` = :customer_Name AND `customer_Password` = :customer_Password AND  `customer_FirstName` = :customer_FirstName AND `customer_LastName` = :customer_LastName AND `customer_Contact` = :customer_Contact AND `customer_Email` = :customer_Email AND `customer_ID` != :customer_ID");
        $stmt->bindParam(':customer_Name', $updateCustomer_Name, PDO::PARAM_STR);
        $stmt->bindParam(':customer_Password', $updateCustomer_Password, PDO::PARAM_STR);
        $stmt->bindParam(':customer_FirstName', $updateCustomer_FirstName, PDO::PARAM_STR);
        $stmt->bindParam(':customer_LastName', $updateCustomer_LastName, PDO::PARAM_STR);
        $stmt->bindParam(':customer_Contact', $updateCustomer_Contact, PDO::PARAM_STR);
        $stmt->bindParam(':customer_Email', $updateCustomer_Email, PDO::PARAM_STR);
        $stmt->bindParam(':customer_ID', $updateCustomer_ID, PDO::PARAM_INT);
        $stmt->execute();

        if ($stmt->rowCount() > 0) {
            // 如果存在重复的 customer_Name，则阻止更新
            echo "<script>alert('Error: User already exists.'); window.location.href = 'http://localhost/projectFYP/dashboardMy/User/index.php';</script>";
            exit();
        }
        ...
        echo "success";
        header("Location: /projectFYP/dashboardMy/User/index.php");
        exit;
